...
export class LocalDatabaseProvider implements ComponentSearchProvider {
  async search(req: ComponentRequirements): Promise<ComponentResult[]> {
    if (req.type === 'inductor') {
      return suggestInductors(req.inductance_uh, req.isat_min_a).map((d) => ({
    ...
    if (req.type === 'capacitor') {
      return suggestCapacitors(req.capacitance_uf, req.voltage_min_v, req.esr_max_mohm).map((d) => ({
        part_number:    d.part_number,
        manufacturer:   d.manufacturer,
    ...
    // mosfet
    return suggestMosfets(req.vds_min_v).map((d) => ({
      part_number:  d.part_number,
      manufacturer: d.manufacturer,
...
// The actual HTTP requests and OAuth2 flow run in the Electron main process via
// electron/digikey-bridge.ts to keep credentials off the renderer.  The renderer
// communicates through window.digikeyAPI, which is injected by the preload script.
