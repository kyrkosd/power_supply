...
// Each entry carries metadata, a LaTeX formula, slider variable definitions,
// and extract functions that read the current value from the live design state.
...
/** Format a number with k/M suffix (no unit). */
...
function mkCapacitanceEq(): EquationEntry {
  return {
    id: 'capacitance',
...
function mkDutyCycleEq(): EquationEntry {
  return {
    id: 'duty_cycle',
...
function mkEfficiencyEq(): EquationEntry {
  return {
    id: 'efficiency',
...
function mkLcCornerEq(): EquationEntry {
  return {
    id: 'lc_corner',
...
function mkMosfetConductionEq(): EquationEntry {
  return {
...
function mkMosfetSwitchingEq(): EquationEntry {
  return {
    id: 'mosfet_switching',
...
/** Look up an equation entry by its id. */
