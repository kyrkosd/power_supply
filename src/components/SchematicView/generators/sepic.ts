...
type SepicLabels = {
  inputInductance:  string
  outputInductance: string
  couplingCap:      string
  outputCap:        string
}

/** Pre-computes all display strings so the component array is pure data. */
function buildLabels(result: DesignResult | null): SepicLabels {
  return {
    inputInductance:  result ? `${formatU(result.inductance  * 1e6, 2, 'µH')}` : '—',
    outputInductance: result?.outputInductance    ? `${formatU(result.outputInductance    * 1e6, 2, 'µH')}` : '—',
    couplingCap:      result?.couplingCapacitance ? `${formatU(result.couplingCapacitance * 1e6, 1, 'µF')}` : '—',
    outputCap:        result ? `${formatU(result.capacitance * 1e6, 1, 'µF')}` : '—',
  }
}

// ── Layout builders ───────────────────────────────────────────────────────────

/** Fixed node junction positions for the SEPIC converter layout. */
...
/**
 * Builds the SEPIC wire routing.
 * The coupling capacitor (Cc) carries the AC current between L1 and L2,
 * allowing the SEPIC to both step up and step down from Vin.
 */
...
export function createSepicSchematic(spec: DesignSpec, result: DesignResult | null): SchematicDefinition {
  ...
  const components = [
    ...
    { id: 'L1',          type: 'inductor' as const,  x: 170, y: 74,  width: 80, height: 116, label: 'L1',   value: lbl.inputInductance,  status: 'normal' as const, meta: 'Input inductor' },
    { id: 'Cc',          type: 'capacitor' as const, x: 320, y: 40,  width: 78, height: 106, label: 'Cc',   value: lbl.couplingCap,       status: 'normal' as const, meta: 'Coupling capacitor' },
    { id: 'L2',          type: 'inductor' as const,  x: 320, y: 186, width: 80, height: 116, label: 'L2',   value: lbl.outputInductance,  status: 'normal' as const, meta: 'Output inductor' },
    ...
    { id: 'Cout',        type: 'capacitor' as const, x: 610, y: 40,  width: 78, height: 106, label: 'Cout', value: lbl.outputCap,          status: 'normal' as const },
