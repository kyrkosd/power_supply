...
export function createBoostSchematic(spec: DesignSpec, result: DesignResult | null): SchematicDefinition {
  ...
  const components = [
    ...
    { id: 'L',    type: 'inductor' as const,  x: 190, y: 74,  width: 80, height: 116, label: 'L',    value: inductanceLabel, status: indStatus },
    { id: 'Q1',   type: 'switch' as const,    x: 340, y: 58,  width: 92, height: 112, label: 'Q1',   value: `D=${duty.toFixed(2)}`, status: swStatus },
    { id: 'D1',   type: (syncMode ? 'switch' : 'diode') as 'switch' | 'diode', x: 340, y: 186, width: 92, height: 72, label: syncMode ? 'Q2' : 'D1', value: syncMode ? 'Sync FET (Rds=8mΩ)' : 'Boost diode', status: 'normal' as const, meta: syncMode ? 'Low-side sync FET' : undefined },
    { id: 'Cout', type: 'capacitor' as const, x: 520, y: 40,  width: 78, height: 106, label: 'Cout', value: `${capacitanceLabel} / ${esrStr}`, status: outStatus },
    ...
    { id: 'Rload', type: 'resistor' as const, x: 680, y: 88,  width: 80, height: 80,  label: 'Rload', value: formatResistance(loadR), status: 'normal' as const },
