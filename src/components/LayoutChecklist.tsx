...
const LoopDiagram: React.FC<{ topology: string }> = ({ topology }) => {
  return (
    <svg className="w-full h-40 bg-gray-900 border border-gray-700 rounded" viewBox="0 0 400 160">
      {/* Generic abstract representation of high di/dt loops */}
      <defs>
...
export const LayoutChecklist: React.FC<LayoutChecklistProps> = ({ topology, primaryIrms, secondaryIrms, mosfetPloss }) => {
  const criticalLoop = useMemo(() => getCriticalLoop(topology), [topology]);
  const priTrace = useMemo(() => calculateTraceWidth(primaryIrms), [primaryIrms]);
  const secTrace = useMemo(() => calculateTraceWidth(secondaryIrms), [secondaryIrms]);
  const copperPourArea = useMemo(() => calculateCopperPour(mosfetPloss), [mosfetPloss]);
    ...
    <div className="space-y-6">
      <div className="bg-gray-800 border border-gray-700 rounded p-4">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-500 mb-2">Based on 1oz copper, external layer, 20°C temp rise allowance.</p>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-900 border border-gray-700 p-3 rounded">
              ...
              <div className="text-lg font-mono text-cyan-400">{priTrace.width_mm.toFixed(2)} mm <span className="text-sm text-gray-500">({priTrace.width_mils.toFixed(0)} mils)</span></div>
            </div>
            <div className="bg-gray-900 border border-gray-700 p-3 rounded">
              <div className="text-xs text-gray-400 mb-1">Secondary Power Path ({secondaryIrms.toFixed(2)} A)</div>
              <div className="text-lg font-mono text-cyan-400">{secTrace.width_mm.toFixed(2)} mm <span className="text-sm text-gray-500">({secTrace.width_mils.toFixed(0)} mils)</span></div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-700 p-3 rounded flex items-center justify-between">
            <div className="text-sm text-gray-300">MOSFET Copper Pour Area<br/><span className="text-xs text-gray-500">For {mosfetPloss.toFixed(2)} W loss to maintain Tj &lt; 100°C</span></div>
            <div className="text-lg font-mono text-amber-400">{copperPourArea > 0 ? `${copperPourArea.toFixed(0)} mm²` : 'Not Required'}</div>
          </div>
        </div>
      </div>
    </div>
